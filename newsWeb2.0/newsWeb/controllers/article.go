...
func(this*ArticleController)ShowIndex(){
	...
	conn,err := redis.Dial("tcp","192.168.230.81:7000")
	if err != nil {
...
func UploadFile(this *ArticleController,filePath string,errHtml string)string{
	...
	response ,_:=client.UploadByBuffer(buffer,ext[1:])
	beego.Info(response)

	return response.RemoteFileId
...
func(this*ArticleController)HandleAddType(){
	...
	//获取数据
	typeName := this.GetString("typeName")
	logoPath := UploadFile(this,"uploadlogo","addType.html")
	typeImagePath := UploadFile(this,"uploadTypeImage","addType.html")
	//校验数据
