...
func init() {
	//注册数据库
	orm.RegisterDataBase("default", "mysql", "root:123456@tcp(192.168.11.150:3306)/pyg")
	//注册表
	orm.RegisterModel(new(User), new(Address), new(TpshopCategory),
		new(Goods), new(GoodsType), new(GoodsSKU), new(GoodsImage),
		new(IndexGoodsBanner), new(IndexTypeGoodsBanner), new(IndexPromotionBanner),
